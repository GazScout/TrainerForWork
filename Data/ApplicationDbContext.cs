...
public DbSet<ExamSubmission> ExamSubmissions => Set<ExamSubmission>();
    ...
    .OnDelete(DeleteBehavior.Cascade);
        ...
        modelBuilder.Entity<HandbookArticle>().HasData(
            ...
            {
                Id = 1,
                Title = "Как приветствовать клиента",
                Content = "Установите зрительный контакт, улыбнитесь и скажите: «Добрый день! Меня зовут [Имя]. Чем я могу вам помочь?»",
                Tags = "клиент,приветствие,общение"
            },
            new HandbookArticle
            {
                Id = 2,
                Title = "Правила возврата товара",
                Content = "Возврат возможен в течение 14 дней при наличии чека и сохранении товарного вида. Технически сложные товары — только при обнаружении дефекта.",
                Tags = "возврат,правила,чек"
            }
        );
    ...
    new TestQuestion
    {
        Id = 1,
        TestId = 1,
        Question = "В течение какого срока возможен возврат товара?",
        OptionsJson = """["7 дней","14 дней","30 дней","60 дней"]""",
        CorrectAnswersJson = "[1]",
    ...
    new TestQuestion
    {
        Id = 2,
        TestId = 1,
        Question = "Какие документы нужны для возврата? (выберите все подходящие)",
        OptionsJson = """["Чек","Паспорт","Заявление","Упаковка"]""",
        CorrectAnswersJson = "[0,2]",
